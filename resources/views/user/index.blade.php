...
@section('content')
    <div class="card shadow-sm  rounded-sm">
        <h4 class="card-header bg-white text-center">Data {{ $level == "dokter" ? "Dokter" : "Pasien" }}</h4>
        <div class="card-body">
            <div class="d-flex justify-content-end">
                <a href="javascript:void[0]" id="btn-add" class="btn btn-success rounded-sm btn-sm px-4 font-weight-bold">Tambah {{ $level == "dokter" ? "Dokter" : "Pasien" }}</a>
            </div>
                    ...
                    <tbody id="table-body">
                        @foreach ($users as $key => $value)
                            <tr id="tr_{{ $value->id_user }}">
                                ...
                                <td class="align-middle">{!! ($value->level == "dokter") ? '<span class="badge text-white bg-primary">Dokter</span>' : '<span class="badge text-white bg-secondary">Pasien</span>' !!}</td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group mb-2">
                    <label for="" class="form-label">Nama {{ $level == "dokter" ? "Dokter" : "Pasien" }}</label>
                ...
                <div class="form-group mb-2">
                    ...
                    <input type="password" id="password" placeholder="Password" class="form-control form-control-sm">
                    <div class="invalid-feedback password"></div>
                </div>                                
                <div class="form-group mb-2">
                    <label for="" class="form-label">Password Konfirmasi</label>
                    <input type="password" id="password_conf" placeholder="Password Konfirmasi" class="form-control form-control-sm">
                    <div class="invalid-feedback password_conf"></div>
                </div>                                
            </div>
    ...
    <script>
        $(document).ready(function() {
            $("#btn-add").click(() => {
                ...
                password_conf.setAttribute("placeholder", "Password Konfirmasi")
                password.value = ""
                password_conf.value = ""
                ...
                $("#btn-save").attr("onclick", "addUser('{{ $level }}')")
                $(".modal-title").text('Tambah Data {{ $level == "dokter" ? "Dokter" : "Pasien" }}')
            })
        ...
        function addUser(level) {
            ...
            data_user.append("username", document.getElementById("username").value)
            data_user.append("no_whatsapp", document.getElementById("no_whatsapp").value)
            data_user.append("password", document.getElementById("password").value)
            data_user.append("password_conf", document.getElementById("password_conf").value)
        ...
        function show(id) {
            ...
            xhttp.onreadystatechange = function () {                            
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    username.value = responses.data.username
                    password.setAttribute('placeholder', "New Password (optional)")
                    password_conf.setAttribute('placeholder', "New Password Confirmation (optional)")
                    password.value = ""
                    password_conf.value  = ""
